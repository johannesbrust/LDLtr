function [step,x1,f1,g1,numf,iExit] ...
              = wolfeLSfgh(prob,wolfeTols,step,stepMax,f,g,H,p,x)
%        [step,x1,f1,g1,numf,iExit] =
%               wolfeLSfgh(prob,wolfeTols,step,stepMax,f,g,H,p,x)
%
%  wolfeLSfgh  finds a step along the search direction  p,  such that
...
%  f,g,H         are the function, gradient and Hessian at x.
...
gp     = g'*p;         pHp  = min(0,p'*H*p);

oldf   = f;            oldg = gp;             oldH = pHp;
f1     = f;            gp1  = gp;
g1     = g;
x1     = x;
...
targetHL =  (wolfeGtol             )*oldH;
targetgU = -(wolfeGtol  + wolfeFtol)*oldg;
targetHU = -(wolfeGtol             )*oldH;
...
%------------------------------------------------------------------
% Commence main loop, entering srchfgh two or more times.
...
while  (1)
  ...
  [first,improved,done,inform,numf,step,sbest] =               ...
      srchfgh( first,                                          ...
               maxf, numf,                                     ...
               stpmax, epsaf,                                  ...
               f0, g0, targetgL, targetgU, targetHL, targetHU, ...
               ftry, gtry,                                     ...
...
%     ==================================================================
%     srchfgh  finds a sequence of improving estimates of a minimizer of
%     the univariate function f(alpha) in the interval (0,alfmax].
%     f(alpha) is a smooth function such that  f(0) = 0  and  f'(0) < 0.
%     srchfgh requires both  f(alpha)  and  f'(alpha)  to be evaluated at
...
%
%     maxf          is an upper limit on the number of times srchfgh is
...
%                   subsequently altered by srchfgh (see below).
%
...
%                   These values may be reduced by srchfgh.
%
%     targetgL,targetHL define the lower bound on the target shifted f'(alfa).
%                   The search is terminated when f(alfa) le 0 and
%                    targetgL + alfa targetHL le f'(alfa)
%                                             le targetgU + alfa*targethU,
%                   with  targetgL < 0 < targetgU and  targetHL < 0 < targetHU
%
%     targetgU,targetHU define the upper bound on the target shifted f'(alfa).
%
%
...
%                   for the new trial alfa, and re-enter srchfgh.
%
...
%
%                   iExit = 5 is never set by srchfgh.
%
...
%
%     numf          counts the number of times srchfgh has been entered
...
%                   approximate minimizer, whenever srchfgh returns
...
%     30 Nov 2019: Switched to the precise Wolfe conditions.
%     ==================================================================
...
if  first
  ...
  found  =          targetgL + alfbst*targetHL  <= gbest  &&  ...
           gbest <= targetgU + alfbst*targetHU;
...
%
% Set the input parameters for srchfgh.
%
